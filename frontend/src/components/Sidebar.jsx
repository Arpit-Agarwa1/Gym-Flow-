...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-4 top-4 z-40 hidden h-[calc(100vh-2rem)] w-64 flex-col overflow-hidden rounded-2xl border border-slate-200/90 bg-white/95 shadow-lg backdrop-blur-xl lg:flex dark:border-white/[0.09] dark:bg-charcoal/90 dark:shadow-panel">
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-neon/[0.06] to-transparent dark:from-neon/[0.04]" />

      <Link
        to="/app"
        className="relative flex shrink-0 items-center gap-3 border-b border-slate-200 px-5 py-5 outline-none ring-offset-2 ring-offset-white focus-visible:ring-2 focus-visible:ring-neon dark:border-white/[0.06] dark:ring-offset-charcoal"
      >
      ...
      <nav className="relative flex-1 space-y-5 overflow-y-auto px-3 pb-4 pt-2 dashboard-scroll">
        {dashboardNavGroups.map((group) => (
          <div key={group.title}>
            <p className="mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-500 dark:text-gray-600">
              {group.title}
            </p>
      ...
      <div className="relative shrink-0 border-t border-slate-200 px-4 py-3 dark:border-white/[0.06]">
        <p className="text-[11px] leading-snug text-slate-500 dark:text-gray-600">
